...
for image_num = 1:length(input_files)
	...
	parfor i=1:length(shell_segments)
		...
		if (eccentricity < 0)
			...
			orientation = mod((orientation + 3 * pi / 2), (2 * pi)) - pi;
			equatoriality = -1 + (1 / (1 + equatoriality));
		end
		x_pos = centres(i, 1);
		y_pos = centres(i, 2);
		...
		fitData(i, :) = [x_pos, y_pos, x_shift, y_shift, orientation, semiminor_axis, psf_variance, height, eccentricity, equatoriality, residual, sum_square_signal];
	end
